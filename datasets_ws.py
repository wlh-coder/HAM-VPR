...
class BaseDataset(data.Dataset):
    ...
    def __init__(self, args, datasets_folder="datasets", dataset_name="pitts30k", split="train"):
        ...
        self.database_utms = np.array(
            [(path.split("@")[1], path.split("@")[2]) for path in self.database_paths]).astype(np.float32)
...
class TripletsDataset(BaseDataset):
    ...
    def __init__(self, args, datasets_folder="datasets", dataset_name="pitts30k", split="train", negs_num_per_query=10):
        ...
        identity_transform = transforms.Lambda(iden_transform)
        # self.identity_transform = transforms.Lambda(identity)
        ...
        # 1. 生成有效查询掩码（排除空列表）
        valid_mask = [len(pos_list) > 0 for pos_list in self.hard_positives_per_query]
        # 2. 过滤硬正例列表（删除空列表）
        self.hard_positives_per_query = [
            pos_list for pos_list, is_valid in zip(self.hard_positives_per_query, valid_mask)
            if is_valid]
        # 3. 同步过滤关联数据（确保索引对齐）
        self.queries_utms = self.queries_utms[valid_mask]  # UTM坐标（NumPy数组按掩码过滤）
        self.queries_paths = [p for p, v in zip(self.queries_paths, valid_mask) if v]  # 查询路径（列表推导式过滤）
        self.queries_num = len(self.queries_paths)  # 更新查询数量
