...
class Adapter(nn.Module):  # Adapter is used to add to the transformer block for global adaptation
    def __init__(self, D_features, mlp_ratio=0.75, act_layer=nn.ReLU, skip_connect=True):
        ...
        self.norm1 = nn.LayerNorm(D_hidden_features, eps=1e-6)
        self.D_fc2 = nn.Linear(D_hidden_features, D_features)
        self.norm2 = nn.LayerNorm(D_features, eps=1e-6)
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        def ffn_residual_func(x: Tensor) -> Tensor:
            return self.ls2(self.mlp(self.norm2(x)) + 0.2 * self.adapter2(
                self.norm2(x)))  # 0.2 is the scaling factor for Parallel adapter
    ...
    def forward_nested(self, x_list: List[Tensor]) -> List[Tensor]:
        ...
        if self.training and self.sample_drop_ratio > 0.0:
            ...
            def ffn_residual_func(x: Tensor, attn_bias=None) -> Tensor:
                return self.mlp(self.norm2(x)) + 0.2 * self.adapter2(self.norm2(x))
